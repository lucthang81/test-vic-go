...
func init() {
	...
	MAP_SYLBOL_NDUP_TO_PRIZE_RATE = map[string]map[int]float64{
		...
		"3": map[int]float64{
			5: 700,
			4: 100, // MATCH_WON_TYPE_AG, ave * 2.2
			3: 2.5,
		},
		"4": map[int]float64{
			5: 500,
			4: 70, // MATCH_WON_TYPE_AG, ave * 2.2
			3: 1.5,
		},
		"5": map[int]float64{
			5: 150, // MATCH_WON_TYPE_AG, ave * 2.2
			4: 40,  // MATCH_WON_TYPE_AG, ave * 2.2
			3: 1,
		},
		"6": map[int]float64{
			5: 50, // MATCH_WON_TYPE_AG, ave * 2.2
			4: 1.5,
			3: 0.5,
		},
		"7": map[int]float64{
	...
	for symbol, mapNdupToRate := range MAP_SYLBOL_NDUP_TO_PRIZE_RATE {
		for nDup, rate := range mapNdupToRate {
			if true {
				...
				sumPayToUser += CalcProbabilityNDupInALine(symbol, nDup) *
					float64(rate) * t
			}
		}
	}
