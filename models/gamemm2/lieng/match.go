...
func (match *ExMatch) ToMapForPlayer(pid int64) map[string]interface{} {
	match.mutex.Lock()
	defer match.mutex.Unlock()
	result := match.Board.ToMapForPlayer(pid)
	return result
}
...
func (match *ExMatch) ReceiveAction(action *gamemm2.Action) {
	...
	case <-t1:
		fmt.Println("ERROR: Lieng match.ReceiveAction TimeOut")
	}
}
...
func Start(match *ExMatch) {
	...
	liengLobbyData := match.Lobby.GameSpecificLobbyData.(*LiengLobbyData)
	dealerSeatI := liengLobbyData.DealerSeat
	dealerPid := int64(0)
	for {
	...
	match.Board = NewLiengBoard(
		listPids,
		match.MapPidToChip,
		dealerPid,
		match.Lobby.Rule.BaseMoney)
	match.Board.StartDealingHoleCards()
	match.mutex.Unlock()
	match.Lobby.UpdateMatchStatus()
	time.Sleep(DURATION_CHANGE_ROUND)

	//
	for _, round := range []string{ROUND_PRE_FLOP} {
		match.mutex.Lock()
		if round == ROUND_PRE_FLOP {
			match.Board.StartPreFlop()
		}
		match.mutex.Unlock()
		match.Lobby.UpdateMatchStatus()
		for {
			if match.Board.InRoundCurrentTurnPlayer == 0 {
				break
			} else {
			...
			ReceivingMoveLoop:
				for {
					...
					case <-timeout:
						match.mutex.Lock()
						err1 := match.Board.MakeMove(Move{
							PlayerId: match.Board.InRoundCurrentTurnPlayer,
							MoveType: MOVE_CHECK})
						match.mutex.Unlock()
						if err1 != nil {
							match.mutex.Lock()
	...
	match.mutex.Lock()
	match.Board.StartShowdown()
	match.Lobby.MatchMaker.Mutex.Lock()
	mapPidToChange := make(map[int64]int64)
	for pid, _ := range match.Board.MapPlayerToWonChip {
		mapPidToChange[pid] = match.Board.MapPlayerToWonChip[pid] -
			match.Board.MapPlayerToLostChip[pid]
	}
	...
	match.Lobby.UpdateMatchStatus()
	time.Sleep(DURATION_CHANGE_ROUND)
	liengLobbyData.DealerSeat = zhelp.GetNextSeat(dealerSeatI, seats)
	select {
